...
function ResetPasswordForm() {
  ...
  const handleReset = async (e: React.FormEvent) => {
    ...
    if (!token) {
      setStatus({ type: "error", message: t('errTokenNotFound') });
      return;
    }
    if (newPassword.length < 6) {
      setStatus({ type: "error", message: t('errTooShort') });
      return;
    }
    if (newPassword !== confirmPassword) {
      setStatus({ type: "error", message: t('errMismatch') });
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStatus({ type: "error", message: data.error || t('errFailed') });
      }
    } catch (error) {
      setStatus({ type: "error", message: t('errServer') });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (!token && !isSuccess && !status.message) {
    return (
      <div className="text-center py-12">
        ...
        <h2 className="text-xl font-display font-bold text-white mb-2 uppercase tracking-wider">{t('invalidLinkTitle')}</h2>
        <p className="text-white/40 mb-8 text-xs font-mono leading-relaxed">{t('invalidLinkDesc')}</p>
  ...
  if (isSuccess) {
    return (
      <div className="text-center py-12 animate-in zoom-in-95 duration-500">
        ...
        <h2 className="text-xl font-display font-bold text-white mb-2 uppercase tracking-wider">{t('successTitle')}</h2>
  ...
  return (
    <>
      <div className="mb-10 text-left">
        ...
        <p className="text-white/45 text-xs font-mono mt-2 leading-relaxed">{t('desc')}</p>
      </div>
      ...
      <form onSubmit={handleReset} className="space-y-6">
        <div className="group">
          <label className="block text-[9px] font-mono font-bold uppercase tracking-widest text-white/45 mb-2 group-focus-within:text-[#ff9e00] transition-colors">{t('newPassLabel')}</label>
        ...
        <div className="group">
          <label className="block text-[9px] font-mono font-bold uppercase tracking-widest text-white/45 mb-2 group-focus-within:text-[#ff9e00] transition-colors">{t('confirmPassLabel')}</label>
          <input 
            type={showPassword ? "text" : "password"} 
        ...
        >
          <div className={`flex items-center justify-center gap-2 transition-opacity duration-300 ${isLoading ? 'opacity-0' : 'opacity-100'}`}>
            {t('saveBtn')} <CheckCircle2 className="w-4 h-4" />
          </div>
          {isLoading && (
    ...
    <div className="bg-[#050505] text-white flex min-h-screen font-sans selection:bg-[#ff9e00] selection:text-black overflow-hidden relative">
      ...
      <div className="hidden lg:flex lg:w-[45%] relative bg-black items-center justify-center p-16 overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-md bg-zinc-950 border border-white/15 rounded-none p-10 shadow-none hover:border-white/25 hover:shadow-[8px_8px_0px_rgba(255,158,0,0.15)] transition-all duration-500 text-center">
           ...
           <h2 className="text-2xl font-display font-bold text-white leading-snug tracking-tight mb-4 uppercase">{t('securityTitle')}</h2>
           <p className="text-white/40 text-xs font-mono leading-relaxed">{t('securityDesc')}</p>
        </div>
      </div>
    </div>
  );
