    ...
    return (
        <section ref={containerRef} className="py-20 px-6 md:px-10 max-w-7xl mx-auto w-full border-t border-white/10 relative z-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 w-full" style={{ perspective: '1000px' }}>
                ...
                {/* Card 1: Wide */}
                <div className={`bento-card-tilt group col-span-1 md:col-span-2 bg-surface/40 backdrop-blur-md border border-white/5 rounded-2xl p-8 flex flex-col justify-between overflow-hidden relative min-h-[300px] ${isEditor ? '' : 'cursor-default'}`} data-cursor="EXPLORE" style={{ transformStyle: 'preserve-3d' }}>
                ...
                {/* Card 2: Tall */}
                <div className={`bento-card-tilt group col-span-1 md:row-span-2 bg-surface/40 backdrop-blur-md border border-white/5 rounded-2xl p-8 flex flex-col justify-end overflow-hidden relative min-h-[400px] md:min-h-full ${isEditor ? '' : 'cursor-default'}`} data-cursor="VIEW" style={{ transformStyle: 'preserve-3d' }}>
                ...
                {/* Card 3: Small Square */}
                <div className="bento-card-tilt group col-span-1 bg-surface/40 backdrop-blur-md border border-white/5 rounded-2xl p-8 flex flex-col justify-between overflow-hidden relative min-h-[250px]" style={{ transformStyle: 'preserve-3d' }}>
                ...
                <div className="bento-card-tilt group col-span-1 bg-surface/40 backdrop-blur-md border border-white/5 rounded-2xl p-8 flex flex-col justify-center items-center overflow-hidden relative min-h-[250px]" style={{ transformStyle: 'preserve-3d' }}>
                    ...
                    <div style={{ transform: isEditor ? 'none' : 'translateZ(40px)' }} className="text-center relative z-10">
                      <h4 className="font-display text-6xl md:text-7xl font-bold text-transparent" style={{ WebkitTextStroke: '1px rgba(250,250,250,0.5)' }}>
                         <EditableText value={theme?.customTexts?.hf_srv4_val || '100%'} field="hf_srv4_val" entity="appearance" isEditor={isEditor} />
                      </h4>
                      <p className="font-mono text-[10px] text-textMuted mt-4 uppercase tracking-widest text-center">
                         <EditableText value={theme?.customTexts?.hf_srv4_label || 'Pixel Perfection'} field="hf_srv4_label" entity="appearance" isEditor={isEditor} />
                      </p>
                    </div>
                </div>
