...
export function FeaturesSection() {
  ...
  return (
    <section id="features" ref={sectionRef} className="bg-[#0a0a0a] pt-24 md:pt-32 pb-0 flex flex-col relative overflow-hidden">
      ...
      <div
        className="w-full border-t border-b border-white/10 grid grid-cols-1 md:grid-cols-2 xl:flex xl:flex-row h-auto xl:h-[450px] relative z-10"
      ...
      >
        {FEATURE_LIST.map((feat, index) => {
          ...
          let borderClasses = '';
          if (index === 0) borderClasses = 'border-b md:border-r xl:border-b-0 xl:border-r border-white/10';
          else if (index === 1) borderClasses = 'border-b xl:border-b-0 xl:border-r border-white/10';
          else if (index === 2) borderClasses = 'border-b md:border-b-0 md:border-r xl:border-b-0 xl:border-r border-white/10';
          else if (index === 3) borderClasses = 'border-b-0 xl:border-r-0 border-white/10';

          return (
            <div
              key={feat.id}
              onMouseEnter={() => window.innerWidth >= 1280 && setActiveFeature(feat.id)}
              onClick={() => setActiveFeature(isActive ? null : feat.id)}
              className={`relative flex-1 p-8 md:p-10 xl:p-12 ${borderClasses} cursor-pointer overflow-hidden transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] bg-[#0a0a0a] ${isActive ? 'xl:flex-[1.4] bg-[#111111]' : ''}`}
            >
