    ...
    >
      ...
      >
        <div slot="poster" className="w-full h-full flex flex-col items-center justify-center bg-zinc-950 text-white/30 gap-3">
          ...
          <span className="text-[9px] font-sans font-medium uppercase tracking-widest">Loading 3D...</span>
        </div>
      </ModelViewer>
    </div>
  );
  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
        {[1, 2, 3, 4, 5, 6].map(i => (
          <div key={i} className="bg-zinc-950 border border-white/10 p-3 shadow-none flex flex-col h-[380px] sm:h-[400px] rounded-md">
            <div className="aspect-[4/3] rounded-md bg-white/5 border border-white/5 shimmer shrink-0"></div>
            <div className="p-3 sm:p-4 pt-4 sm:pt-5 flex-1 flex flex-col space-y-3">
              <div className="h-4 bg-white/5 border border-white/5 rounded-md w-3/4 shimmer"></div>
              <div className="h-3 bg-white/5 border border-white/5 rounded-md w-full shimmer"></div>
              <div className="h-3 bg-white/5 border border-white/5 rounded-md w-4/5 shimmer"></div>
              <div className="mt-auto flex gap-2 pt-4">
                <div className="h-10 bg-white/5 border border-white/5 rounded-md flex-1 shimmer"></div>
                <div className="h-10 w-11 bg-white/5 border border-white/5 rounded-md shrink-0 shimmer"></div>
              </div>
            </div>
          </div>
        ))}
  ...
  if (filteredItems.length === 0) {
    return (
      <div className="py-20 sm:py-24 flex flex-col items-center justify-center text-center animate-enter" style={{ animationDelay: '150ms' }}>
        <div className="w-16 h-16 sm:w-20 sm:h-20 bg-zinc-900 rounded-md flex items-center justify-center mb-5 sm:mb-6 border border-white/10 text-white/30">
      ...
      <div key={activeTab} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
        {filteredItems.map((item: any, index: number) => (
          <AnimatedCard key={item.id} delay={index * 60}>
            <div className="group bg-zinc-950 rounded-md p-3 border border-white/10 hover:border-[#ff9e00]/40 transition-all duration-300 flex flex-col relative h-full">
              <div className="relative aspect-[4/3] rounded-md overflow-hidden bg-zinc-900 shrink-0 border border-white/5">
                {item.projectType === '3d' ? (
                ...
                <div className="absolute top-3 left-3 bg-black/85 border border-white/10 px-2.5 py-1.5 rounded-md text-[8px] sm:text-[9px] font-sans font-medium uppercase tracking-wider text-white/80 flex items-center gap-1.5 pointer-events-none">
              ...
              <div className="p-3 sm:p-4 pt-4 sm:pt-5 flex-1 flex flex-col">
                <div className="flex justify-between items-start gap-2 mb-1.5 sm:mb-2">
                  <h3 className="font-sans font-medium text-sm text-white line-clamp-1 uppercase tracking-wider">{item.title}</h3>
                  {item.itemType === 'certificate' && (
                    <div className="w-4 h-4 rounded-full bg-[#ff9e00] text-black flex items-center justify-center shrink-0 text-[8px] font-medium shadow-sm" title="Verified Credential">
                ...
                {item.itemType !== 'certificate' && (() => {
                  ...
                  return tags.length > 0 ? (
                    <div className="flex flex-wrap gap-1.5 mt-2.5">
                      {tags.map((tag: string) => (
                        <span key={tag} className="inline-flex items-center px-2 py-0.5 bg-zinc-900 text-[#ff9e00] text-[9px] font-sans font-medium uppercase tracking-wider rounded-md border border-white/5">
                          {tag}
                        </span>
                      ))}
                    </div>
                ...
                <div className="mt-5 flex items-center gap-2">
                  <button
                    ...
                    className="flex-1 bg-zinc-900 text-white border border-white/10 rounded-md py-2.5 text-[10px] font-sans font-medium uppercase tracking-widest hover:bg-zinc-800 transition-colors flex items-center justify-center gap-2 active:scale-95"
                  >
                    <Edit3 className="w-3 h-3" /> Edit Work
                  </button>
                  <button
                    onClick={() => confirmDelete(item.id, item.title, item.itemType)}
                    className="w-10 h-10 shrink-0 bg-zinc-900 border border-white/10 text-white/40 rounded-md flex items-center justify-center hover:bg-rose-950/20 hover:text-rose-400 hover:border-rose-900/30 transition-all active:scale-95"
                    title="Delete"
                  >
