        ...
        <div className="lg:col-span-9 flex flex-col min-h-[500px]">
            <div className="p-8 border-b border-white/10 bg-white/5 flex items-center justify-between">
                <h2 className="font-display font-bold uppercase text-2xl tracking-wider text-[#ff9e00]">
                    {searchQuery ? t('searchResults') : (activeCategory ? CATEGORIES.find(c => c.id === activeCategory)?.name : t('allArticles'))}
                </h2>
                <span className="font-mono text-xs opacity-50 hidden sm:block">
                    {t('showingResults', { count: filteredArticles.length, noun: filteredArticles.length === 1 ? t('resultSingular') : t('resultPlural') })}
                </span>
            </div>
            ...
            {filteredArticles.length > 0 ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-12 text-center border-b border-white/10">
                    ...
                    <h3 className="font-display text-2xl font-bold uppercase tracking-wider mb-2">{t('noResults')}</h3>
                    <p className="font-mono text-sm text-white/50 mb-8 max-w-md">{t('noResultsDesc')}</p>
                    <button 
                    ...
                    >
                        <ArrowLeft className="w-4 h-4" /> {t('clearFilters')}
                    </button>
                </div>
            )}
