...
export default function AuraKineticVideoShowcase({ data, theme, isEditor }: any) {
  ...
  return (
    <section className="py-24 px-4 w-full bg-white relative overflow-hidden">
      {/* Animated Aura */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 rounded-full blur-[100px] opacity-20 animate-[spin_10s_linear_infinite] pointer-events-none" />
      ...
      <div className="w-full max-w-6xl mx-auto flex flex-col items-center relative z-10">
        <motion.div initial="hidden" {...{ [animationTrigger]: "visible" }} viewport={{ once: true }}
          ...
          className="w-full aspect-video bg-white/50 backdrop-blur-3xl shadow-[0_30px_60px_rgba(0,0,0,0.1)] relative z-20 pointer-events-auto rounded-[2rem] overflow-hidden border border-white p-4"
        >
        ...
        <div className="mt-8 text-center">
          <h2 className="text-3xl font-black bg-clip-text text-transparent bg-gradient-to-r from-blue-500 to-purple-500 tracking-tighter">
