  ...
  return (
    <div className="text-center mb-16">
      ...
      <h1 className="text-4xl md:text-6xl lg:text-7xl font-sans font-medium text-white tracking-tight leading-[1.15] mb-6">
        {t('titlePrefix')} <span className="text-white/40 italic font-light">{t('titleHighlight')}</span><br/>{t('titleSuffix')}
      </h1>
      ...
      <div className="flex items-center justify-center gap-4 max-w-xs mx-auto mb-16">
        <span className={`text-xs font-medium font-sans uppercase tracking-widest ${billingCycle === 'monthly' ? 'text-white' : 'text-white/40'}`}>{t('monthly')}</span>
        <button 
        ...
        <span className={`text-xs font-medium font-sans uppercase tracking-widest ${billingCycle === 'yearly' ? 'text-white' : 'text-white/40'}`}>{t('yearly')}</span>
        <span className="bg-emerald-500 text-black text-[9px] font-sans font-medium px-2 py-0.5 animate-pulse">{t('save')}</span>
      </div>
    </div>
  );
}
