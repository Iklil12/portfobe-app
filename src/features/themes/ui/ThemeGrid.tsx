...
export function ThemeGrid({ themes, state, actions }: { themes: any[], state: ThemesState, actions: ThemesActions }) {
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 mb-24 animate-enter" style={{ animationDelay: '140ms' }}>
                    ...
                    >
                        ...
                        `}>
                            ...
                            <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10 flex flex-col justify-between p-4 pointer-events-none">
                                ...
                                <div className="flex justify-end pointer-events-auto">
                                    <button
                                        ...
                                        className={`w-8.5 h-8.5 rounded-md flex items-center justify-center transition-all duration-300 border
                                            ${isFavorite
                                ...
                                <div className="flex justify-between items-end transform translate-y-0 md:translate-y-2 group-hover:translate-y-0 transition-transform duration-300 pointer-events-auto">
                                    <span className="text-white font-sans font-medium uppercase tracking-wider text-[11px] truncate pr-4 drop-shadow-md">
                                        {theme.name}
                                    </span>
                                    ...
                                    <button
                                        ...
                                        className={`px-3.5 py-2 rounded-md text-[10px] font-sans font-medium uppercase tracking-wider transition-all duration-300 flex items-center gap-1.5 active:scale-95
                                            ${isActive
                            ...
                            {!theme.isAvailable && isHovered && (
                                <div className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none">
                                    <div className="bg-black/90 border border-white/10 px-5 py-2.5 rounded-md shadow-[0_10px_35px_rgba(0,0,0,0.85)]">
                                        <span className="text-white font-sans font-medium text-[9px] uppercase tracking-widest flex items-center gap-2">
                        ...
                        <div className="flex justify-between items-center px-1">
                            <div className="flex items-center gap-2 min-w-0">
                                {/* Creator Avatar */}
                                <div className="w-5 h-5 rounded-md bg-zinc-900 border border-white/10 text-white/50 flex items-center justify-center shrink-0">
                                    <Palette className="w-3 h-3" />
                                </div>

                                {/* Theme Name */}
                                <span className="text-xs font-sans font-medium text-white uppercase tracking-wider truncate">
                                    {theme.name}
                                </span>

                                {/* Badges */}
                                {theme.isPro && (
                                    <span className="bg-[#ff9e00] text-black text-[9px] font-sans font-medium px-2 py-0.5 rounded-md uppercase tracking-wider shrink-0">
                                        Pro
                                    </span>
                                )}
                                {isActive && (
                                    <span className="bg-[#ff9e00]/10 border border-[#ff9e00]/30 text-[#ff9e00] text-[9px] font-sans font-medium px-2 py-0.5 rounded-md uppercase tracking-wider shrink-0">
                                        Active
                                    </span>
                                )}
                            </div>
                            ...
                            {/* Stats */}
                            <div className="flex items-center gap-3 text-white/40 text-[10px] font-sans shrink-0">
                                <span
                                    onClick={(e) => { e.stopPropagation(); toggleFavorite(theme.id); }}
            ...
            <div className="group flex flex-col gap-3">
                <div className="relative w-full aspect-video border border-dashed border-white/10 rounded-md flex flex-col items-center justify-center bg-[#050505] hover:border-white/20 transition-all duration-300 group cursor-default">
                    <div className="w-12 h-12 bg-zinc-900 border border-white/10 rounded-md flex items-center justify-center mb-4 group-hover:scale-105 transition-transform duration-300 text-white/40 group-hover:text-white">
                ...
                <div className="flex justify-between items-center px-1">
                    <div className="flex items-center gap-2">
                        <div className="w-5 h-5 rounded-md bg-zinc-900 border border-white/10 flex items-center justify-center text-white/40">
                            <Clock className="w-3 h-3" />
                        </div>
                        <span className="text-xs font-sans font-medium text-white/30 uppercase tracking-wider">Other Themes...</span>
                    </div>
                </div>
            </div>
        </div>
