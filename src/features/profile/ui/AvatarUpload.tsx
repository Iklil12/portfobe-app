...
export function AvatarUpload({ state, actions }: AvatarUploadProps) {
  ...
  return (
    <div className="relative mb-8 border-b border-white/10 pb-8 sm:pb-10 pt-32 sm:pt-40">
      ...
      <div className="absolute -top-[25px] -left-[25px] -right-[25px] sm:-top-[41px] sm:-left-[41px] sm:-right-[41px] md:-top-[43px] md:-left-[43px] md:-right-[43px] h-40 sm:h-48 bg-zinc-900 border-b border-white/10 flex items-center justify-center group rounded-md">
        ...
        <div className="relative z-10 p-4">
          <div className="absolute inset-0 bg-white/5 rounded-md blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-700"></div>
          <LazyImage
            src="/portfo.be.webp"
      ...
      <div className="flex flex-col sm:flex-row items-end justify-between gap-4 relative z-10">
        ...
        <div className="flex flex-col items-center sm:items-start w-full sm:w-auto">
          ...
          >
            <div className="absolute -inset-1 bg-white/10 rounded-md blur-md opacity-0 group-hover:opacity-10 transition duration-500"></div>
            <div className="relative w-full h-full rounded-md border-2 border-zinc-950 overflow-hidden bg-zinc-900">
              {isUploading && (
            ...
            {isPro && (
              <div className={`absolute -bottom-1 -right-1 w-6 h-6 rounded-md border border-zinc-950 flex items-center justify-center text-black z-40 ${isSupreme ? 'bg-purple-400' : 'bg-[#ff9e00]'}`}>
          ...
          <div className="flex flex-col items-center sm:items-start text-center sm:text-left">
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 sm:gap-3 mb-1.5">
              <h2 className="text-xl sm:text-2xl font-sans font-medium text-white uppercase tracking-tight">
                {firstName ? `${firstName} ${lastName}` : fullName}
              </h2>
              {/* Creator Tag - HANYA MUNCUL JIKA PRO/SUPREME */}
              {isPro && (
                <span className={`text-[9px] font-sans font-medium px-2 py-0.5 rounded-md flex items-center gap-1 uppercase tracking-wider ${isSupreme ? 'bg-purple-500/10 text-purple-400 border border-purple-500/20' : 'bg-[#ff9e00]/10 text-[#ff9e00] border border-[#ff9e00]/20'}`}>
                  <span className={`w-1.5 h-1.5 rounded-md animate-pulse ${isSupreme ? 'bg-purple-400' : 'bg-[#ff9e00]'}`}></span> {isSupreme ? 'Supreme Creator' : 'Pro Creator'}
                </span>
              )}
            </div>
        ...
        <div className="flex items-center justify-center sm:justify-end gap-3 w-full sm:w-auto mt-6 sm:mt-0">
          <Link href="/dashboard/projects" className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-4 py-2 rounded-md border border-white/10 bg-zinc-900/50 hover:bg-[#ff9e00]/10 hover:border-[#ff9e00]/30 hover:text-[#ff9e00] text-white/70 text-[11px] font-sans font-medium transition-all active:scale-95">
