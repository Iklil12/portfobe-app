...
export default function ThemesPage() {
  const t = useTranslations('DashboardThemes');
  const FILTER_TABS = [
    { id: 'all',       label: t('allThemes'),   icon: LayoutGrid },
    { id: 'free',      label: t('free'),         icon: Gift },
    { id: 'pro',       label: t('pro'),          icon: Crown },
        ...
        {filteredThemes.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="font-sans font-medium text-white mb-2">
              {activeFilter === 'favorites' ? t('noFavorites') : t('noThemes')}
            </p>
            <p className="text-white/60 text-xs font-sans max-w-xs">
              {activeFilter === 'favorites' ? t('noFavDesc') : t('noThemeDesc')}
            </p>
          </div>
        ) : (
