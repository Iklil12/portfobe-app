...
export function EnterpriseHeroSection() {
    ...
    const cards = [
        ...
        { type: 'color', bg: 'bg-[#18181b]', content: <div className="flex flex-col items-center justify-center leading-none"><span className="text-white font-black text-5xl md:text-6xl tracking-tighter">{t('create')}</span><span className="text-[#ff9e00] font-medium text-lg md:text-xl tracking-[0.2em] mt-2">{t('beyond')}</span></div> },
        { type: 'image', src: '/img/abstract_shapes_1781107379628.webp' },
        { type: 'color', bg: 'bg-[#0a0a0a] border border-white/10', content: <div className="flex flex-col items-center justify-center"><span className="w-12 h-[2px] bg-white/50 mb-4"></span><span className="text-white font-mono text-xs md:text-sm tracking-[0.4em] uppercase">{t('vanguard')}</span><span className="w-12 h-[2px] bg-white/50 mt-4"></span></div> },
    ...
    return (
        <section className="bg-[#050505] text-white font-mono overflow-x-hidden w-full relative z-30 pt-24 pb-20 border-t border-white/10">
            ...
            <div className="w-full max-w-5xl mx-auto px-6 text-center mb-16 md:mb-20">
                <span className="font-mono text-[10px] tracking-[0.3em] uppercase text-white/30 block mb-6">[ {t('socialProof')} ]</span>
