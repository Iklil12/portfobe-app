            ...
            <div className="flex flex-col @lg:flex-row gap-16 @lg:gap-24">
                ...
                <motion.div initial="hidden" {...{ [animationTrigger]: "visible" }} viewport={{ once: true, amount: 0 }} variants={fadeUp} className="w-full @lg:w-1/3">
                    <h2 className={`font-sans font-semibold tracking-tight text-[#111] text-4xl @md:text-5xl @lg:text-6xl mb-6`}>
                        <EditableText value={theme?.customTexts?.editorial_services_t1 || 'Core'} field="editorial_services_t1" entity="appearance" isEditor={isEditor} as="span" maxLength={20} /> <EditableText value={theme?.customTexts?.editorial_services_t2 || 'Expertise'} field="editorial_services_t2" entity="appearance" isEditor={isEditor} as="span" className="font-serif italic text-slate-400" maxLength={20} />
                    </h2>
                    <p className="font-sans text-sm @md:text-base text-slate-500 max-w-sm leading-relaxed">
                        <EditableText value={theme?.customTexts?.editorial_services_sub || 'A disciplined approach to digital creation, focusing on strategy, design, and seamless execution.'} field="editorial_services_sub" entity="appearance" isEditor={isEditor} as="span" maxLength={150} />
                    </p>
                </motion.div>
                    ...
                    {displayServices.map((service, i) => (
                        ...
                        >
                            ...
                            <div className="w-full @md:w-1/3 mb-4 @md:mb-0">
                                <span className="font-sans text-xs font-bold text-[var(--hl)] uppercase tracking-widest block mb-2">0{i + 1} //</span>
                                <h3 className="font-serif italic text-2xl @md:text-3xl text-[#111] pr-4">
                                    <EditableText value={theme?.customTexts?.[`editorial_svc_${service.id}_title`] || service.defaultTitle} field={`editorial_svc_${service.id}_title`} entity="appearance" isEditor={isEditor} as="span" maxLength={40} />
                                </h3>
                            </div>
                            ...
                            <div className="w-full @md:w-2/3">
                                <p className="font-sans text-sm @md:text-base text-slate-500 leading-relaxed group-hover:text-[#111] transition-colors duration-300">
                                    <EditableText value={theme?.customTexts?.[`editorial_svc_${service.id}_desc`] || service.defaultDesc} field={`editorial_svc_${service.id}_desc`} entity="appearance" isEditor={isEditor} as="span" maxLength={150} />
                                </p>
                            </div>
                        </motion.div>
                    ))}
