    ...
    return (
        <section className="w-full min-h-[100vh] py-32 px-6 md:px-16 bg-[#111] text-white flex flex-col justify-center relative stack-card">
            <h2 className="text-5xl md:text-7xl font-bold uppercase tracking-tight mb-16 relative z-10">
                <EditableText entity="appearance" field="layeredmonolith_skills_title" value={getCustomText('layeredmonolith_skills_title', 'Core Capabilities')} isEditor={isEditor} maxLength={40} as="span" />
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {[1, 2, 3, 4].map((num) => {
                    ...
                    const val = parseInt(getCustomText(`layeredmonolith_skill_prof_${num}`, defaultProficiency) || '0', 10);
                        ...
                        <div key={num} className="bg-black/40 p-8 border border-white/10 backdrop-blur-md mb-6 relative z-10 hover:bg-black/60 transition-all">
                            <div className="flex justify-between items-center mb-4 text-3xl font-bold">
                                <span>
                                    <EditableText 
                                        entity="appearance" 
                                        field={`layeredmonolith_skill_name_${num}`} 
                                        value={getCustomText(`layeredmonolith_skill_name_${num}`, defaultName)} 
                                        isEditor={isEditor} 
                                        maxLength={40} 
                                        as="span" 
                                    />
                                ...
                                <span>
                                    <EditableText 
                                        entity="appearance" 
                                        field={`layeredmonolith_skill_prof_${num}`} 
                                        value={getCustomText(`layeredmonolith_skill_prof_${num}`, defaultProficiency)} 
                                        isEditor={isEditor} 
                                        maxLength={3} 
                                        as="span" 
                                    />%
                            ...
                            <div className={`w-full h-2 ${isEditor ? '' : 'overflow-hidden'} bg-white/20`}>
                                <motion.div 
                                    initial={{ width: 0 }}
