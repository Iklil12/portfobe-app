...
export default function AnalyticsPage() {
  ...
  const handleLocked = useCallback(() => showToast({ message: t('upgradeToPro'), id: "range-lock", icon: "fa-lock" }), []);

  const handleComingSoon = useCallback((e?: React.MouseEvent) => {
    if (e) e.stopPropagation();
    showToast({ message: t('comingSoon'), id: "coming-soon-analytics", icon: "fa-clock" });
  }, []);

  const RANGES = [
    { id: '1d', label: t('range1d'), pro: false },
    { id: '7d', label: t('range7d'), pro: false },
    { id: '30d', label: t('range30d'), pro: true },
    { id: 'all', label: t('rangeAll'), pro: true },
  ];
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-8 pb-32 selection:bg-[#ff9e00]/30 selection:text-white">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10 animate-enter">
        <div>
          <h1 className="text-3xl font-sans font-medium text-white tracking-tight uppercase mb-1.5">{t('pageTitle')}</h1>
          <p className="text-xs font-sans text-white/60">{t('pageDesc')}</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-8">
        ...
        <TopLocations 
          ...
          title={t('widgetTopCountries')} subtitle={t('visitorOriginCountry')} 
        />
        ...
        <TopLocations 
          ...
          title={t('widgetTopCities')} subtitle={t('visitorOriginCity')} 
        />
      </div>
