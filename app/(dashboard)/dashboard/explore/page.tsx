...
export default function ExplorePage() {
  const t = useTranslations('DashboardExplore');
    ...
    <main className="min-h-screen relative overflow-hidden pb-24 selection:bg-[#ff9e00]/30 selection:text-white">
      ...
      <div className="max-w-5xl mx-auto p-4 sm:p-6 md:p-10 relative z-10">
        ...
        <div className="animate-enter mb-12" style={{animationDelay:'0ms'}}>
          ...
          <h1 className="text-3xl font-sans font-medium text-white tracking-tight uppercase mb-3">{t('title')}<span className="text-white/60">{t('titleHighlight')}</span>
          </h1>
          <p className="text-white/50 text-xs font-sans max-w-lg leading-relaxed">{t('desc')}</p>
        </div>
        ...
        <div className="animate-enter" style={{animationDelay:'200ms'}}>
          <div className="flex items-end justify-between mb-5">
            <div>
              <h2 className="font-sans font-medium text-white text-xs">{t('communityPreview')}</h2>
              <p className="text-white/50 text-[9px] font-sans mt-1">{t('initialRelease')}</p>
            </div>
            <span className="px-2.5 py-1 bg-zinc-900 text-white/60 text-[9px] font-sans font-medium rounded-md border border-white/5">{t('soonBadge')}</span>
          </div>
          ...
          <div className="relative rounded-md overflow-hidden border border-white/10">
            ...
            <div className="absolute inset-0 z-10 backdrop-blur-md bg-black/70 flex flex-col items-center justify-center px-4">
              ...
              <p className="font-sans font-medium text-white text-xs mb-1.5">{t('notOpen')}</p>
              <p className="text-white/60 font-mono text-[10px] text-center max-w-xs leading-relaxed">{t('notOpenDesc')}</p>
            </div>
          ...
          <div className="bg-zinc-900/50 border border-white/10 rounded-md p-6 sm:p-8 relative overflow-hidden">
            <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-5">
              <div>
                ...
                <h3 className="font-sans font-medium text-white text-sm mb-1">{t('beFirst')}</h3>
                <p className="text-white/60 font-mono text-xs leading-relaxed max-w-xl">{t('beFirstDesc')}</p>
              </div>
              <button
                onClick={() => {}}
