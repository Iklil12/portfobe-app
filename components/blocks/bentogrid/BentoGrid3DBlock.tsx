        ...
        <div className="grid auto-rows-[minmax(120px,auto)] gap-4 @lg:gap-6 grid-cols-1 w-full">
            ...
            >
                <div className="flex flex-col @md:flex-row justify-between items-start @md:items-end mb-4 @md:mb-8 gap-3 @md:gap-4">
                    <div>
                        <h3 className="text-lg @md:text-3xl font-black text-white tracking-tight flex items-center gap-2 @md:gap-3">
                            <i className="fas fa-cube text-[var(--hl)]"></i> 3D Models
                        </h3>
                        <p className="custom-body text-xs @md:text-sm text-slate-400 mt-1 @md:mt-2">Interactive spatial assets</p>
                    </div>
                ...
                <div className="flex flex-col gap-6">
                    {items3D.map((p: any, i: number) => (
                        <div key={p.id || i} className={`group relative ${cardStyleClass} ${cardRadiusClass} p-4 hover:border-[var(--hl)] transition-all duration-300 hover:shadow-[0_0_50px_rgba(var(--hl-rgb),0.15)]`}>
                            <div className="relative w-full aspect-square @md:aspect-video rounded-xl @md:rounded-[24px] overflow-hidden bg-[#121214]">
                                <Interactive3DViewer mediaUrl={p.mediaUrl} bgColor="#121214" alwaysShowControls />
                                <div className="absolute inset-0 border-2 border-transparent group-hover:border-[var(--hl)]/30 rounded-xl @md:rounded-[24px] pointer-events-none transition-colors duration-300"></div>
                            </div>
                            <div className="flex justify-between items-center mt-3 @md:mt-6 px-1 @md:px-4">
                                <div className="flex flex-col">
                                    <span className="text-[8px] @md:text-[10px] font-bold uppercase tracking-[0.3em] text-[var(--hl)] mb-0.5 @md:mb-1 opacity-60">Asset 0{i+1}</span>
                                    <h4 className="font-bold text-white text-base @md:text-2xl group-hover:text-[var(--hl)] transition-colors">{p.title}</h4>
                                    {p.description && <p className="text-sm text-slate-400 mt-2 line-clamp-2 max-w-2xl">{p.description}</p>}
                                </div>
                                <div className="flex w-8 h-8 @md:w-12 @md:h-12 rounded-full border border-white/10 items-center justify-center text-slate-500 group-hover:bg-[var(--hl)] group-hover:text-black transition-all shrink-0">
