...
export function TopBanner({ isLoading, topBanner }: TopBannerProps) {
  ...
  return (
    <div className={`animate-page-load delay-200 shrink-0 border-b flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 px-6 sm:px-10 py-3 transition-all duration-500 relative overflow-hidden pr-12 sm:pr-16 z-20 ${baseBg}`}>
      ...
      <div className={`absolute top-0 right-0 w-48 h-48 blur-3xl opacity-20 -translate-y-1/2 translate-x-1/4 rounded-md pointer-events-none ${isCritical ? 'bg-rose-500' : 'bg-amber-500'}`}></div>

      {/* Tombol Close (X) */}
      <button 
        onClick={() => setDismissed([...dismissed, topBanner.id])}
        className="absolute top-2.5 right-4 sm:top-1/2 sm:-translate-y-1/2 sm:right-6 w-7 h-7 flex items-center justify-center rounded-md bg-zinc-950 border border-white/10 hover:bg-zinc-900 text-white/60 hover:text-white transition-colors z-20"
      ...
      <div className="flex items-start sm:items-center gap-3.5 relative z-10">
        <div className={`mt-0.5 sm:mt-0 w-8 h-8 rounded-md flex items-center justify-center shrink-0 border shadow-none ${iconColor}`}>
          <i className={`fas ${topBanner.icon} text-xs`}></i>
        </div>
        <div className="flex flex-col">
          <h3 className="text-[11px] sm:text-xs font-sans font-medium mb-0.5 text-white">{topBanner.title}</h3>
          <p className="text-[10px] sm:text-[11px] font-sans font-medium text-white/50 leading-snug">{topBanner.desc}</p>
        </div>
      </div>
      
      {topBanner.btnText && (
        <Link href={topBanner.link} className={`relative z-10 shrink-0 px-5 py-2.5 rounded-md font-sans font-medium text-[10px] sm:text-[11px] transition-all hover:-translate-y-0.5 active:scale-95 flex items-center justify-center gap-2 w-full sm:w-auto mt-2 sm:mt-0 ${btnClass}`}>
