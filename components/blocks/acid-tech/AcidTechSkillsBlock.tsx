...
export function AcidTechSkillsBlock({ theme, isEditor }: any) {
        ...
        <section className="w-full py-24 px-6 md:px-12 bg-black border-y border-[#00ff00]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {[1, 2, 3, 4].map((num) => {
                    ...
                    const val = parseInt(getCustomText(`acidtech_skill_prof_${num}`, defaultProficiency) || '0', 10);
                    ...
                    return (
                        <div key={num} className="border border-[#00ff00]/30 p-6 mb-6 hover:bg-[#00ff00]/10 transition-colors">
                            <div className="flex justify-between items-center mb-4 text-2xl font-mono text-[#00ff00]">
                                <span>
                                    <EditableText 
                                        entity="appearance" 
                                        field={`acidtech_skill_name_${num}`} 
                                        value={getCustomText(`acidtech_skill_name_${num}`, defaultName)} 
                                        isEditor={isEditor} 
                                        maxLength={40} 
                                        as="span" 
                                    />
                                ...
                                <span>
                                    <EditableText 
                                        entity="appearance" 
                                        field={`acidtech_skill_prof_${num}`} 
                                        value={getCustomText(`acidtech_skill_prof_${num}`, defaultProficiency)} 
                                        isEditor={isEditor} 
                                        maxLength={3} 
                                        as="span" 
                                    />%
                            ...
                            <div className={`w-full h-2 ${isEditor ? '' : 'overflow-hidden'} bg-[#00ff00]/20`}>
                                <motion.div 
                                    initial={{ width: 0 }}
