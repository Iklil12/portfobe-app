    ...
    >
      ...
      <div className="text-center max-w-3xl mx-auto mb-20">
        <motion.div variants={auraAnim} className={`inline-flex items-center gap-2 px-4 py-2 ${radiusClass} glass-panel mb-6`}>
          <span className="text-xs font-medium text-slate-300">
            <EditableText value={customTexts.spatial_services_label || 'Services'} field="spatial_services_label" entity="appearance" isEditor={isEditor} as="span" maxLength={20} />
          </span>
        </motion.div>
        <motion.h2 variants={auraAnim} className="font-semibold tracking-[-0.03em] text-gradient leading-tight text-4xl @md:text-5xl">
          <EditableText value={customTexts.spatial_services_title || 'What I bring to the table.'} field="spatial_services_title" entity="appearance" isEditor={isEditor} as="span" maxLength={60} />
        </motion.h2>
      </div>
      ...
      <motion.div variants={staggerContainer} className="grid grid-cols-1 @md:grid-cols-2 @lg:grid-cols-3 gap-6">
        {services.map((service, i) => {
          const isVisible = customTexts[`spatial_svc_${service.id}_visible`] !== 'false';
            ...
            >
              ...
              <div className="relative">
                ...
                <h3 className="text-xl font-semibold text-white mb-3">
                  <EditableText value={customTexts[`spatial_svc_${service.id}_title`] || service.defaultTitle} field={`spatial_svc_${service.id}_title`} entity="appearance" isEditor={isEditor} as="span" maxLength={40} />
                  {!isVisible && " [HIDDEN]"}
                </h3>
                <p className="text-slate-400 text-sm leading-relaxed group-hover:text-slate-300 transition-colors">
                  <EditableText value={customTexts[`spatial_svc_${service.id}_desc`] || service.defaultDesc} field={`spatial_svc_${service.id}_desc`} entity="appearance" isEditor={isEditor} as="span" maxLength={120} />
                </p>
              </div>
            </motion.div>
          );
