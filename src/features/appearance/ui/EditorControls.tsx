...
export function EditorControls({ state, actions }: { state: ThemeEditorState, actions: ThemeEditorActions }) {
  const {
    ...
    splashScreen, isLoading, customTexts, pageBlocks, rawProjects
  } = state;
      ...
      {hasVideoShowcase && (
        <div className="mb-8">
          ...
          <div className="flex flex-col gap-4">
              ...
              <div className="relative">
                ...
                >
                  <span className="text-xs font-medium truncate">
                    {customTexts?.showcase_video_id 
                      ? videoProjects.find((vp: any) => vp.id === customTexts.showcase_video_id)?.title || 'Unknown Video'
                    ...
                    <div className="max-h-56 overflow-y-auto custom-scrollbar">
                      <button
                        ...
                        className={`w-full text-left px-3 py-2.5 text-xs transition-colors flex items-center justify-between group
                          ${!customTexts?.showcase_video_id ? 'bg-[#ff9e00]/10 text-[#ff9e00]' : 'text-white/60 hover:bg-white/5 hover:text-white'}`}
                      >
                        <span className="truncate italic">None (Hide Video)</span>
                        {!customTexts?.showcase_video_id && <Check className="w-3.5 h-3.5" />}
                      </button>
                      
                      {videoProjects.map((vp: any) => {
                        const isSelected = customTexts?.showcase_video_id === vp.id;
                        return (
                          <button
                            key={vp.id}
                            onClick={() => {
            ...
            <button 
              onClick={() => updateCustomText('showcase_autoplay', customTexts?.showcase_autoplay === 'true' ? 'false' : 'true')}
            ...
            >
              ...
              <div className={`w-8 h-4 shrink-0 rounded-full relative transition-colors duration-300 ${customTexts?.showcase_autoplay === 'true' ? 'bg-[#ff9e00]' : 'bg-white/10'}`}>
                <div className={`w-3 h-3 rounded-full absolute top-0.5 left-0.5 transition-transform duration-300 shadow-sm ${customTexts?.showcase_autoplay === 'true' ? 'bg-black translate-x-4' : 'bg-white/40 translate-x-0'}`}></div>
              </div>
            </button>
          </div>
          <div className="w-full h-px bg-white/5 my-6"></div>
      ...
      <div className="mb-6">
        ...
        {!isLoading && (
          <div className="flex flex-col gap-2">
            ...
            <button 
              onClick={() => updateCustomText('smooth_scroll', customTexts?.smooth_scroll === 'true' ? 'false' : 'true')}
              className="w-full p-3 rounded-md border border-transparent hover:border-white/5 bg-transparent hover:bg-white/5 transition-all flex items-center justify-between group text-sm"
            >
              <div className="flex items-center gap-2.5">
              ...
              <div className={`w-8 h-4 shrink-0 rounded-full relative transition-colors duration-300 ${customTexts?.smooth_scroll === 'true' ? 'bg-[#ff9e00]' : 'bg-white/10'}`}>
                <div className={`w-3 h-3 rounded-full absolute top-0.5 left-0.5 transition-transform duration-300 ${customTexts?.smooth_scroll === 'true' ? 'bg-black translate-x-4' : 'bg-white/40 translate-x-0'}`}></div>
              </div>
            </button>

            <button 
              onClick={() => updateCustomText('custom_cursor_enabled', customTexts?.custom_cursor_enabled === 'true' ? 'false' : 'true')}
              className="w-full p-3 rounded-md border border-transparent hover:border-white/5 bg-transparent hover:bg-white/5 transition-all flex items-center justify-between group text-sm"
            >
              <div className="flex items-center gap-2.5">
              ...
              <div className={`w-8 h-4 shrink-0 rounded-full relative transition-colors duration-300 ${customTexts?.custom_cursor_enabled === 'true' ? 'bg-[#ff9e00]' : 'bg-white/10'}`}>
                <div className={`w-3 h-3 rounded-full absolute top-0.5 left-0.5 transition-transform duration-300 ${customTexts?.custom_cursor_enabled === 'true' ? 'bg-black translate-x-4' : 'bg-white/40 translate-x-0'}`}></div>
              </div>
            </button>

            {customTexts?.custom_cursor_enabled === 'true' && (
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className={`py-2 px-2 border rounded text-[10.5px] font-mono flex items-center justify-center gap-1.5 transition-all ${
                      (customTexts?.custom_cursor_type || 'circle-dot') === 'circle-dot'
                  ...
                  <button
                    ...
                    className={`py-2 px-2 border rounded text-[10.5px] font-mono flex items-center justify-center gap-1.5 transition-all ${
                      customTexts?.custom_cursor_type === 'solid-dot'
                        ? 'border-[#ff9e00] text-[#ff9e00] bg-[#ff9e00]/5'
                        : 'border-white/10 text-white/60 hover:border-white/20 hover:text-white bg-transparent'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`py-2 px-2 border rounded text-[10.5px] font-mono flex items-center justify-center gap-1.5 transition-all ${
                      customTexts?.custom_cursor_type === 'neon-pointer'
                  ...
                  <button
                    ...
                    className={`py-2 px-2 border rounded text-[10.5px] font-mono flex items-center justify-center gap-1.5 transition-all ${
                      customTexts?.custom_cursor_type === 'paper-plane'
