...
export default function SupportPage() {
  ...
  const handleSend = async () => {
    ...
    setIsSending(true);
    const toastId = toast.loading(t('msgSending'));
    ...
    try {
      ...
      if (res.ok) {
        toast.success(t('msgSuccess'), { id: toastId });
        setFormData(prev => ({ ...prev, message: '' }));
      } else {
        toast.error(t('msgFailed'), { id: toastId });
      }
    } catch (err) {
      toast.error(t('errNetwork'), { id: toastId });
    } finally {
      setIsSending(false);
    }
  };
  ...
  const waMessage = encodeURIComponent(t('waGreeting'));
      ...
      <main className="max-w-6xl mx-auto px-6 pt-32 pb-24 relative z-10">
        ...
        <div className="text-center mb-16">
          ...
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-display font-bold text-white tracking-tight leading-[1.15] mb-6">
            {t('headlinePrefix')} <span className="text-white/40 italic font-light">{t('headlineHighlight')}</span> {t('headlineSuffix')}
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto items-stretch">
          ...
          <div className="bg-zinc-950 p-8 border border-white/10 hover:border-white/20 transition-all flex flex-col justify-between group">
            <div>
              ...
              <h3 className="text-lg font-display font-bold text-white mb-3">{t('kbTitle')}</h3>
              <p className="text-xs text-white/50 font-mono tracking-wide leading-relaxed mb-8">{t('kbDesc')}</p>
            </div>
            <Link href="/learn/knowledge-base" className="text-[10px] font-mono font-bold uppercase tracking-widest text-[#ff9e00] hover:text-white transition-colors flex items-center gap-2">
              {t('openArticles')} <ArrowRight className="w-3.5 h-3.5" />
            </Link>
          </div>
          ...
          <div className="bg-zinc-950 p-8 border border-white/10 hover:border-white/20 transition-all flex flex-col justify-between group">
            <div>
              ...
              <h3 className="text-lg font-display font-bold text-white mb-3">{t('emailTitle')}</h3>
              <p className="text-xs text-white/50 font-mono tracking-wide leading-relaxed mb-8">{t('emailDesc')}</p>
            </div>
            <a href="mailto:ikliluluyun@ritions.com" className="text-[10px] font-mono font-bold uppercase tracking-widest text-[#ff9e00] hover:text-white transition-colors flex items-center gap-2">
              {t('sendMessage')} <Send className="w-3.5 h-3.5" />
            </a>
          </div>
          ...
          <div className={`relative p-8 border transition-all flex flex-col justify-between ${isPro ? 'bg-zinc-950 border-[#ff9e00]' : 'bg-zinc-950/40 border-white/5 opacity-85'}`}>
            <div>
              <div className="flex justify-between items-start mb-6">
                ...
                {isPro ? (
                  <span className="text-[#ff9e00] text-[9px] font-mono uppercase tracking-widest border border-[#ff9e00]/30 px-2 py-0.5">{t('priorityActive')}</span>
                ) : (
                  <span className="text-white/30 text-[9px] font-mono uppercase tracking-widest border border-white/10 px-2 py-0.5 flex items-center gap-1"><Lock className="w-2.5 h-2.5" /> {t('locked')}</span>
                )}
              </div>
              <h3 className="text-lg font-display font-bold text-white mb-3">{t('waTitle')}</h3>
        ...
        <div className="mt-20 max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-5 gap-8">
          ...
          <div className="md:col-span-2 bg-zinc-950 p-10 border border-white/10 h-fit">
            <h4 className="text-xl font-display font-bold text-white mb-2">{t('businessInfo')}</h4>
            <p className="text-xs text-white/50 font-mono tracking-wide mb-8">{t('businessDesc')}</p>
            
            <div className="space-y-6 text-sm font-mono text-white/70">
              <div>
                <div className="text-[10px] font-bold uppercase tracking-widest text-white/40 mb-1">{t('legalName')}</div>
                <strong className="text-white">Portfobe</strong>
              </div>
              <div>
                <div className="text-[10px] font-bold uppercase tracking-widest text-white/40 mb-1">{t('physicalAddress')}</div>
                <div>
                  Jl. jl.Raya Telang, Kamal, Bangkalan, jawa Timur, Indonesia 69162<br/>
                </div>
              </div>
              <div>
                <div className="text-[10px] font-bold uppercase tracking-widest text-white/40 mb-1">{t('emailLabel')}</div>
                <a href="mailto:ikiluluyun@ritions.com" className="text-[#ff9e00] hover:underline">ikiluluyun@ritions.com</a>
              </div>
              <div>
                <div className="text-[10px] font-bold uppercase tracking-widest text-white/40 mb-1">{t('phoneLabel')}</div>
          ...
          <div className="md:col-span-3 bg-zinc-950 p-10 border border-white/10">
            <h4 className="text-xl font-display font-bold text-white mb-2">{t('formTitle')}</h4>
            <p className="text-xs text-white/50 font-mono tracking-wide mb-8">{t('formDesc')}</p>

            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="text-[10px] font-mono font-bold uppercase tracking-widest text-white/40 mb-2 block">{t('nameLabel')}</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-[10px] font-mono font-bold uppercase tracking-widest text-white/40 mb-2 block">{t('emailFormLabel')}</label>
                  <input
                    type="email"
              ...
              <div>
                <label className="text-[10px] font-mono font-bold uppercase tracking-widest text-white/40 mb-2 block">{t('messageLabel')}</label>
                <textarea
                  rows={4}
              ...
              >
                {isSending ? t('sending') : t('submitForm')}
              </button>
            </div>
          </div>
        </div>
