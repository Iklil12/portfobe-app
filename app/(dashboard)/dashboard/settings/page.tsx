...
type TabId = 'account' | 'billing' | 'integrations';
...
function SettingsContent() {
  ...
  const TABS = [
    { id: 'account' as TabId, label: t('tabAccount'), icon: Shield },
    { id: 'billing' as TabId, label: t('tabBilling'), icon: CreditCard },
    { id: 'integrations' as TabId, label: t('tabIntegrations'), icon: Plug, comingSoon: true },
  ];
    ...
    <main className="min-h-screen relative overflow-hidden pb-24 bg-zinc-950">
      ...
      <div className="max-w-5xl mx-auto p-4 sm:p-6 md:p-10 relative z-10">
        ...
        <div className="mb-8 sm:mb-10 animate-enter text-center md:text-left" style={{animationDelay: '100ms'}}>
          ...
          <p className="text-xs font-sans text-white/60 max-w-lg mx-auto md:mx-0">{t('pageDesc')}</p>
        </div>
            ...
            return (
              ...
              >
                ...
                {'comingSoon' in tab && tab.comingSoon && (
                  <span className="text-[8px] font-sans font-medium px-1.5 py-0.5 rounded-md ml-1 bg-zinc-950 border border-white/5 text-white/50">{t('tabSoon')}</span>
                )}
              </button>
            );
          })}
            ...
            <>
              ...
              {activeTab === 'integrations' && (
                <div className="flex flex-col items-center justify-center py-24 text-center border border-dashed border-white/10 rounded-md bg-zinc-900/10">
                  ...
                  <h3 className="text-xs font-sans font-medium text-white mb-2">{t('comingSoonTitle')}</h3>
                  <p className="text-[10px] font-sans text-white/60 max-w-md leading-relaxed">{t('comingSoonDesc')}</p>
                </div>
              )}
            </>
          )}
...
export default function SettingsPage() {
  return (
    <Suspense fallback={<div className="flex h-screen items-center justify-center font-mono text-xs text-white/60 bg-zinc-950">Loading...</div>}>
      <SettingsContent />
    </Suspense>
  );
}
