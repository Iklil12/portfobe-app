...
export default function AppearancePage() {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FAFAFA] animate-in fade-in duration-500">
    ...
    <main className="h-[calc(100vh-80px)] md:h-screen flex flex-col lg:flex-row bg-[#FAFAFA] font-sans overflow-hidden animate-in fade-in duration-1000">
      ...
      <div className="w-full lg:w-5/12 h-full bg-white border-r border-slate-200 flex flex-col z-10 shadow-[5px_0_30px_rgba(0,0,0,0.03)] lg:shadow-none relative">
          ...
          <div className="flex gap-2">
            {subdomain && (
               <a 
                 ...
                 target="_blank" 
                 rel="noreferrer"
                 className="px-4 py-3 bg-slate-100 text-slate-700 rounded-full text-xs font-bold uppercase tracking-widest hover:bg-slate-200 transition-colors shadow-sm active:scale-95 flex items-center justify-center"
        ...
        <div className="p-6 md:p-8 overflow-y-auto pb-32">
          ...
          {/* TOGGLE SPLASH SCREEN (Selalu Muncul) */}
          <div className="mb-10 animate-in fade-in slide-in-from-bottom-4 duration-500 delay-300">
            <div className="flex items-center justify-between border border-slate-200 p-5 rounded-2xl bg-slate-50/50 hover:bg-slate-50 transition-colors shadow-sm cursor-pointer" onClick={() => setSplashScreen(!splashScreen)}>
              ...
              <button 
                className={`w-14 h-7 rounded-full relative transition-colors duration-300 ease-in-out shrink-0 outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 ${splashScreen ? 'bg-slate-900' : 'bg-slate-300'}`}
      ...
      {/* --- KANAN: LIVE PREVIEW --- */}
      <div className="hidden lg:flex lg:w-7/12 bg-[#F1F5F9] relative flex-col items-center justify-center p-6 sm:p-10 overflow-hidden gap-6 animate-in fade-in zoom-in-95 duration-700">
