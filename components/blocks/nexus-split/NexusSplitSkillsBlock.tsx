        ...
        <section className="w-full py-24 md:py-32 px-8 border-b border-white/10 bg-black relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {[1, 2, 3, 4].map((num) => {
                    ...
                    const val = parseInt(getCustomText(`nexussplit_skill_prof_${num}`, defaultProficiency) || '0', 10);
                    ...
                    return (
                        <div key={num} className="flex flex-col py-6 border-b border-white/10 hover:border-[var(--hl)] transition-colors group">
                            <div className="flex justify-between items-center mb-4 text-2xl font-bold uppercase text-white group-hover:text-[var(--hl)] transition-colors">
                                <span>
                                    <EditableText 
                                        entity="appearance" 
                                        field={`nexussplit_skill_name_${num}`} 
                                        value={getCustomText(`nexussplit_skill_name_${num}`, defaultName)} 
                                        isEditor={isEditor} 
                                        maxLength={40} 
                                        as="span" 
                                    />
                                ...
                                <span>
                                    <EditableText 
                                        entity="appearance" 
                                        field={`nexussplit_skill_prof_${num}`} 
                                        value={getCustomText(`nexussplit_skill_prof_${num}`, defaultProficiency)} 
                                        isEditor={isEditor} 
                                        maxLength={3} 
                                        as="span" 
                                    />%
