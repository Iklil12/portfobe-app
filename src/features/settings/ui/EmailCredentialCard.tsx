    ...
    <div className="bg-zinc-900/40 p-6 sm:p-8 md:p-10 rounded-md border border-white/10 shadow-none hover:border-[#ff9e00]/30 transition-all duration-300 animate-enter" style={{animationDelay: '250ms'}}>
      ...
      <div className="flex flex-col sm:flex-row items-center gap-4">
        <div className="relative w-full">
          ...
          <input type="email" value={session?.user?.email || "Loading..."} disabled className="w-full pl-12 pr-5 py-3.5 sm:py-4 rounded-md border border-white/5 bg-zinc-950 text-white/40 font-mono text-xs outline-none cursor-not-allowed" />
        </div>
        <button 
          onClick={() => setShowEmailModal(true)} 
          disabled={isOAuthLinked}
          className={`w-full sm:w-auto px-8 py-3.5 sm:py-4 text-[11px] font-sans font-medium uppercase tracking-widest rounded-md transition-all flex items-center justify-center shrink-0
            ${isOAuthLinked 
