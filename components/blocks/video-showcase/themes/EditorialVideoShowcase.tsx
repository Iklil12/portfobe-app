...
export default function EditorialVideoShowcase({ data, theme, isEditor }: any) {
  ...
  if (!featuredVideo) {
    if (isEditor) {
      return (
        <section className="py-20 px-4 w-full bg-[#fdfdfc] flex items-center justify-center border-y border-subtle">
          <div className="text-center p-8">
            <h3 className="text-[#111111] font-serif italic text-2xl mb-2">Editorial Video</h3>
            <p className="text-xs font-mono uppercase tracking-widest text-[#111111]/50">Select a video project</p>
          </div>
        </section>
      );
    }
  ...
  return (
    <section className="py-24 px-4 md:px-12 w-full bg-[#fdfdfc] text-[#111111] border-y border-subtle">
      <div className="w-full max-w-7xl mx-auto flex flex-col md:flex-row gap-12 items-start">
        <motion.div
          ...
          className="md:w-1/3 pt-4 border-t-2 border-subtle"
        >
          <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#111111]/50 mb-6 block">
          ...
          <h2 className="text-5xl md:text-7xl font-serif italic tracking-tight text-[#111111] mb-6 leading-[0.9]">
          ...
          {featuredVideo.description && (
            <p className="text-[#111111]/70 leading-relaxed font-serif text-lg first-letter:text-5xl first-letter:font-bold first-letter:mr-1 first-letter:float-left">{featuredVideo.description}</p>
          )}
        ...
        <motion.div
          ...
          className={`md:w-2/3 w-full aspect-video ${cardRadiusClass} overflow-hidden relative z-20 pointer-events-auto ${cardStyleClassLight} group-hover:-translate-y-2 transition-all duration-700`}
        >
          <div className="w-full h-full relative pointer-events-auto">
             {isAutoPlay ? (
               <div className="w-full h-full opacity-90 hover:opacity-100 transition-all pointer-events-auto">
                 <UniversalPlayer mediaUrl={featuredVideo.mediaUrl} title={featuredVideo.title} autoPlayMode={true} className="w-full h-full bg-black" />
               </div>
             ) : (
               <UniversalPlayer mediaUrl={featuredVideo.mediaUrl} title={featuredVideo.title} className="w-full h-full bg-black" />
             )}
          </div>
        </motion.div>
      </div>
