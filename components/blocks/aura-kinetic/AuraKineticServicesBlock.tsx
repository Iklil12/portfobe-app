            ...
            <div className="mb-20 text-center max-w-3xl mx-auto">
                <motion.div initial="hidden" {...{ [animationTrigger]: "visible" }} variants={cardVariants} viewport={{ once: true, amount: 0.5 }}>
                    <div className={`inline-block glass-panel ${btnShape} px-4 py-1.5 text-xs font-medium tracking-widest text-white mb-6 border border-white/20`}>
                        <EditableText value={theme?.customTexts?.aura_services_label || 'Expertise'} field="aura_services_label" entity="appearance" isEditor={isEditor} as="span" maxLength={30} />
                    </div>
                    <h2 className="font-heading text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight">
                        <EditableText value={theme?.customTexts?.aura_services_title || 'What we do best.'} field="aura_services_title" entity="appearance" isEditor={isEditor} as="span" maxLength={60} />
                    </h2>
                </motion.div>
            </div>
                    ...
                    >
                        ...
                        <div className="relative">
                            ...
                            <h3 className="font-heading text-2xl font-bold mb-4 text-white">
                                <EditableText value={theme?.customTexts?.[`aura_svc_${service.id}_title`] || service.defaultTitle} field={`aura_svc_${service.id}_title`} entity="appearance" isEditor={isEditor} as="span" maxLength={40} />
                            </h3>
                            
                            <p className="font-body text-gray-400 leading-relaxed group-hover:text-gray-300 transition-colors">
                                <EditableText value={theme?.customTexts?.[`aura_svc_${service.id}_desc`] || service.defaultDesc} field={`aura_svc_${service.id}_desc`} entity="appearance" isEditor={isEditor} as="span" maxLength={150} />
                            </p>
                        </div>
                    </motion.div>
                ))}
