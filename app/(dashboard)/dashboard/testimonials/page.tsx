...
export default function TestimonialsPage() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!validMimeTypes.includes(file.type)) {
      showToast({ message: "t('unsupportedFormat')", id: "err-testimonial-img-type", icon: "fa-exclamation" });
      return;
    }

    const maxImageSize = 5 * 1024 * 1024;
    if (file.size > maxImageSize) {
      showToast({ message: "t('maxSize')", id: "err-testimonial-img-size", icon: "fa-exclamation" });
      return;
    }
    ...
    try {
      ...
      if (res.ok && data.secure_url) {
        ...
        showToast({ message: "t('uploadQuick')", id: "up-ok", icon: "fa-bolt" });
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        resetForm();
        showToast({ message: t('saveSuccess', { action: editingId ? 'updated' : 'added' }), id: "save-success", icon: "fa-check-circle" });
        fetchTestimonials();
      } else {
        const data = await res.json();
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        fetchTestimonials();
        showToast({ message: "t('deleteSuccess')", id: "del-success", icon: "fa-trash" });
      } else {
        const data = await res.json();
        ...
        {testimonialToDelete && createPortal(
          <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4">
            ...
            <div className="relative z-10 w-full max-w-[320px] md:max-w-[400px] mx-auto bg-zinc-950 border border-white/10 rounded-md shadow-[0_45px_100px_rgba(0,0,0,0.9)] p-6 md:p-8 flex flex-col text-center">
              ...
              <h3 className="text-base md:text-lg font-sans font-medium text-white mb-2">{t('deleteModalTitle')}</h3>
              ...
              <div className="flex flex-row gap-2 md:gap-3 w-full">
                ...
                >
                  ...
                  {isDeleting ? t('deletingBtn') : t('deleteModalBtn')}
                </button>
                <button 
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 gap-4 animate-enter">
              <div>
                <h1 className="text-xl sm:text-2xl md:text-3xl font-sans font-medium uppercase tracking-wider text-white flex items-center gap-3">
                  <MessageSquare className="w-6 h-6 text-[#ff9e00]" />{t('title')}</h1>
                <p className="text-xs font-sans text-white/60 mt-2">{t('desc')}</p>
              </div>
              <button
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="flex flex-col sm:flex-row gap-8">
                    ...
                    <div className="flex flex-col items-center sm:items-start gap-3">
                      <label className="text-[9px] font-sans font-medium text-white/60">{t('photoOpt')}</label>
                      <input 
                        type="file" 
                      ...
                      >
                        ...
                        {formData.avatarUrl ? (
                          ...
                          <div className="flex flex-col items-center text-white/60 group-hover:text-[#ff9e00] transition-colors">
                            ...
                            <span className="text-[8px] font-sans font-medium">{t('upload')}</span>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex-1 space-y-5">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                          <label className="block text-[9px] font-sans font-medium text-white/60 mb-2 ml-1">{t('clientName')}</label>
                        ...
                        <div>
                          <label className="block text-[9px] font-sans font-medium text-white/60 mb-2 ml-1">{t('position')}</label>
                      ...
                      <div>
                        <label className="block text-[9px] font-sans font-medium text-white/60 mb-2 ml-1">{t('content')}</label>
                      ...
                      <div>
                        <label className="block text-[9px] font-sans font-medium text-white/60 mb-2 ml-1">{t('starRating')}</label>
                  ...
                  <div className="pt-6 border-t border-white/5 flex justify-end">
                    <button disabled={isAdding} type="submit" className="bg-[#ff9e00] hover:bg-[#ffaa22] text-black px-8 py-3.5 rounded-md font-sans font-medium text-xs active:scale-95 transition-all disabled:opacity-50 flex items-center gap-1.5 shadow-md">
                      ...
                      {isAdding ? t('savingBtn') : t('saveBtn')}
                    </button>
                  </div>
                </form>
              </div>
            ...
            <div className="space-y-4">
              {testimonials.length === 0 ? (
                <div className="text-center py-16 bg-[#050505] rounded-md border border-white/10 border-dashed animate-enter">
                  ...
                  <h3 className="font-sans font-medium text-white mb-1">{t('noTestiTitle')}</h3>
                  <p className="text-white/60 text-xs font-sans max-w-xs mx-auto mb-6">{t('noTestiDesc')}</p>
                ...
                testimonials.map((t, index) => (
                  <div key={t.id} className={`bg-zinc-950 p-5 sm:p-6 rounded-md border transition-all duration-300 ${t.isVisible ? 'border-white/10 hover:border-white/20 hover:bg-white/[0.01]' : 'border-white/5 opacity-40 bg-zinc-900/50'} flex flex-col sm:flex-row gap-5 animate-enter`} style={{animationDelay: `${index * 100}ms`}}>
                    ...
                    <div className="hidden sm:flex flex-col gap-2 justify-start border-l border-white/5 pl-5 w-[130px] shrink-0">
                      <button onClick={() => handleEditClick(t)} className="px-3 py-2 flex items-center justify-center gap-1.5 rounded-md transition-all text-[10px] font-sans font-medium w-full bg-zinc-900 border border-white/10 text-white/70 hover:bg-zinc-800"><PenTool className="w-3.5 h-3.5" /> {t('edit')}</button>
