...
export function LinksHeader({ state, actions }: LinksHeaderProps) {
  const { hasChanges, isSaving, isAdding, userPlan, linkCount } = state;
      ...
      <div className="text-center md:text-left">
        <div className="flex items-center justify-center md:justify-start gap-3 mb-5 sm:mb-6">
          ...
          {userPlan === 'PRO' ? (
             ...
             <span className="bg-slate-50 text-slate-400 text-[9px] font-black px-2.5 py-1 rounded-md tracking-widest uppercase border border-slate-200">FREE</span>
          )}
        </div>
        ...
        {userPlan === 'FREE' ? (
           <div className="flex items-center justify-center md:justify-start gap-3 mt-4">
              <div className="w-32 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                <div 
                  className={`h-full transition-all duration-1000 ${isFull ? 'bg-rose-500' : 'bg-slate-900'}`} 
                  style={{ width: `${Math.min((linkCount / 1) * 100, 100)}%` }}
                ></div>
              </div>
              <span className={`text-[10px] font-black ${isFull ? 'text-rose-500' : 'text-slate-500'}`}>
                {linkCount}/1 Tautan Digunakan
              </span>
           </div>
        ) : (
          <p className="text-xs sm:text-sm font-medium text-slate-500 max-w-lg">Kelola direktori tautan tanpa batas sebagai Pro Creator.</p>
        )}
        {isFull && (
          <p className="text-[10px] font-bold text-rose-500 mt-3 flex items-center justify-center md:justify-start gap-1.5 animate-pulse">
            <i className="fas fa-crown text-[9px]"></i> Limit tercapai. Upgrade ke PRO untuk menambah lebih banyak link.
          </p>
        )}
      </div>
