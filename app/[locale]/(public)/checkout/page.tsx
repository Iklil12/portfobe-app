...
function CheckoutContent() {
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      setCouponError('Network error occurred');
    } finally {
  ...
  useEffect(() => {
    if (appliedCoupon) {
      if (appliedCoupon.minPurchase !== null && appliedCoupon.minPurchase !== undefined && baseTotal < appliedCoupon.minPurchase) {
        setAppliedCoupon(null);
        setCouponError(`Coupon "${appliedCoupon.code}" removed as minimum purchase of Rp ${appliedCoupon.minPurchase.toLocaleString('id-ID')} is not met.`);
      } else if (appliedCoupon.allowedPlan && appliedCoupon.allowedPlan !== 'ALL' && plan !== appliedCoupon.allowedPlan) {
        setAppliedCoupon(null);
        setCouponError(`Coupon "${appliedCoupon.code}" removed as it is only valid for the ${appliedCoupon.allowedPlan.toUpperCase()} plan`);
      }
    }
      ...
      <div className="min-h-screen bg-[#050505] text-white font-sans flex flex-col items-center justify-center p-6 relative overflow-hidden">
        ...
        <div className="bg-zinc-950 border border-white/10 p-10 max-w-md w-full text-center space-y-8 animate-in fade-in zoom-in-95 relative z-10">
          ...
          <div className="space-y-3">
            <h2 className="text-xl font-display font-bold text-white uppercase tracking-wider">{t('planUnavailableTitle')}</h2>
            <p className="text-xs font-mono text-white/50 leading-relaxed" dangerouslySetInnerHTML={{ __html: t('planUnavailableDesc', { plan }).replace(/\*\*(.*?)\*\*/g, '<span class="font-bold text-white uppercase">$1</span>') }}></p>
          </div>
          
          {activePlans.length > 0 ? (
            <div className="space-y-4 pt-2 text-left">
              <p className="text-[10px] font-mono font-bold text-white/30 uppercase tracking-widest text-center">{t('chooseOtherPlan')}</p>
              <div className="flex flex-col gap-2.5">
                {activePlans.map((pCode) => (
                  ...
                  >
                    <span>{pricing[pCode].name} {t('planSuffix')}</span>
      ...
      <div className="max-w-6xl mx-auto px-6 pt-12 grid grid-cols-1 lg:grid-cols-12 gap-12 items-start animate-in fade-in slide-in-from-bottom-4 duration-75 relative z-10">
        ...
        <div className="lg:col-span-8 space-y-8">
          <div>
            ...
            <h1 className="text-3xl md:text-4xl font-display font-bold text-white tracking-tight">{t('title')}</h1>
          </div>
          ...
          <div className="bg-zinc-950 border border-white/10 rounded-none relative">
            <div className={`p-6 md:p-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8 ${duration === 'yearly' ? 'border-b border-white/10' : ''}`}>
              ...
              <div className="flex-1 w-full">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    <h2 className="text-xl font-display font-bold text-white uppercase tracking-wider">{pricing[plan].name} {t('planSuffix')}</h2>
                    <p className="text-xs font-mono text-white/40 mt-0.5">{t('professionalLicense')}</p>
                  </div>
                </div>

                <div className="space-y-2 relative max-w-md">
                  <label className="text-[9px] font-mono font-bold text-white/40 uppercase tracking-widest">{t('billingDuration')}</label>
                  <div className="relative">
                    ...
                    >
                      {duration === 'yearly' 
                        ? `${t('yearlyLabel')} - ${formatIDR(pricing[plan].yearly.price)}${t('moSuffix')} (${t('save20Badge')})` 
                        : `${t('monthlyLabel')} - ${formatIDR(pricing[plan].monthly.price)}${t('moSuffix')}`}
                    ...
                    {isDurationDropdownOpen && (
                      <>
                        ...
                        <div className="absolute top-full left-0 right-0 mt-2 bg-zinc-950 border border-white/10 rounded-none shadow-2xl z-50 overflow-hidden py-1 animate-in fade-in slide-in-from-top-2 duration-200">
                          ...
                          >
                            <span>{t('monthlyLabel')} - {formatIDR(pricing[plan].monthly.price)}{t('moSuffix')}</span>
                          ...
                          >
                            <span>
                              {t('yearlyLabel')} - {formatIDR(pricing[plan].yearly.price)}{t('moSuffix')} 
                              <span className="ml-2 text-[8px] bg-emerald-500/10 text-emerald-400 px-2 py-0.5 rounded-none uppercase font-bold tracking-wider border border-emerald-500/20">{t('save20Badge')}</span>
                            </span>
                ...
                {isRecalculating ? (
                  ...
                  <>
                    <div className="text-[10px] font-mono font-bold text-white/30 line-through mb-1">
                      {formatIDR(selectedPricing.original)}{t('monthSuffix')}
                    </div>
                    <div className="flex items-baseline gap-1 md:justify-end">
                      ...
                      <span className="text-xs font-mono text-white/40">{t('moSuffix')}</span>
                    </div>
                    {duration === 'yearly' && (
                      <div className="mt-3 inline-block bg-emerald-500/10 text-emerald-400 text-[9px] font-mono font-bold px-2.5 py-1 border border-emerald-500/20 animate-in fade-in slide-in-from-bottom-2">
                        {t('saveText')} {formatIDR(selectedPricing.original - selectedPricing.price)}{t('moSuffix').toUpperCase()}
                      </div>
                    )}
                  </>
                )}
            ...
            {duration === 'yearly' && plan === 'supreme' && (
              <div className="bg-emerald-500/[0.02] p-4 md:px-8 border-t border-emerald-500/10 flex gap-3 items-center rounded-none">
                ...
                <p className="text-xs font-mono text-emerald-400" dangerouslySetInnerHTML={{ __html: t('bonusDomain').replace(/\*\*(.*?)\*\*/g, '<strong class="text-white font-bold">$1</strong>') }}></p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="lg:col-span-4">
          <div className="bg-zinc-950 border border-white/10 p-6 md:p-8 sticky top-24 min-h-[500px] flex flex-col justify-between">
            {isRecalculating ? (
              ...
              <>
                <div>
                  <h2 className="text-sm font-mono font-bold text-white/30 uppercase tracking-widest mb-6">{t('billingDetails')}</h2>
                  
                  <div className="space-y-6 mb-8">
                    <div className="flex justify-between items-start">
                      <div>
                        <h4 className="text-xs font-mono font-bold text-white uppercase tracking-wider mb-1">{pricing[plan].name} {t('planSuffix')}</h4>
                        <p className="text-[10px] font-mono text-white/40">{t('durationLabel')} {duration === 'yearly' ? t('months12') : t('month1')}</p>
                      </div>
                    ...
                    {duration === 'yearly' && plan === 'supreme' && (
                      <div className="flex justify-between items-start animate-in fade-in slide-in-from-bottom-2">
                        <div>
                          <h4 className="text-xs font-mono font-bold text-white uppercase tracking-wider mb-1">{t('customDomainLabel')}</h4>
                          <p className="text-[10px] font-mono text-white/40">{t('firstYearLabel')}</p>
                        </div>
                        <div className="text-right">
                  ...
                  <div className="border-t border-white/10 pt-6 mb-8">
                    {appliedCoupon && (
                      <div className="flex justify-between items-end mb-2 text-white/40">
                        <span className="text-xs font-mono font-bold uppercase">{t('subtotalLabel')}</span>
                        <span className="text-xs font-mono line-through">{formatIDR(baseTotal)}</span>
                      </div>
                    )}
                    <div className="flex justify-between items-end mb-1">
                      <span className="text-xs font-mono font-bold uppercase tracking-wider text-white/40">{t('totalDueLabel')}</span>
                    ...
                    <p className="text-right text-[10px] font-mono font-bold text-emerald-400">
                      {t('savedLabel')} {formatIDR(savings)}!
                    </p>
                  </div>
                  ...
                  {appliedCoupon ? (
                    <div className="flex items-center justify-between bg-emerald-500/5 border border-emerald-500/20 px-4 py-3.5 mb-6 animate-in fade-in zoom-in duration-300">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-[9px] font-mono text-emerald-400/80 mt-0.5">{t('discountApplied', { amount: appliedCoupon.discountType === 'PERCENTAGE' ? `${appliedCoupon.discountValue}%` : formatIDR(appliedCoupon.discountValue) })}</p>
                        </div>
                      </div>
                    ...
                    >
                      <Tag className="w-3.5 h-3.5" /> {t('haveCoupon')}
                    </button>
                  ) : (
                ...
                <div>
                  ...
                  <div className="mt-8 space-y-6 border-t border-white/5 pt-6">
                    <div className="flex items-center justify-center gap-2 text-[10px] font-mono font-bold text-white/40 uppercase tracking-wider">
                      <Shield className="w-4 h-4 text-white/30" /> {t('guarantee')}
                    </div>
                    
                    <div className="flex flex-col items-center gap-1.5">
                      <div className="flex items-center gap-1">
                        <span className="text-[10px] font-mono font-bold text-white/50 mr-1 uppercase">{t('excellent')}</span>
                      ...
                      <div className="text-[9px] font-mono text-white/30">
                        <span className="font-bold text-white/40">4.8/5</span> {t('reviewsText')} <span className="font-bold text-white/50">Trustpilot</span>
                      </div>
                    </div>
                  </div>
                </div>
