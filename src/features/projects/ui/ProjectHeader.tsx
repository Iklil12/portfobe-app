          ...
          {isLoading ? (
            ...
            >
              <div className="space-y-2">
                <div className="w-24 h-2 bg-white/5 rounded-md animate-pulse shimmer"></div>
                <div className="w-full h-1 bg-white/5 rounded-md animate-pulse shimmer"></div>
              </div>
              <div className="space-y-2">
                <div className="w-24 h-2 bg-white/5 rounded-md animate-pulse shimmer"></div>
                <div className="w-full h-1 bg-white/5 rounded-md animate-pulse shimmer"></div>
            ...
            >
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="w-full h-1 bg-zinc-900 border border-white/10 rounded-md overflow-hidden">
                  <motion.div 
                    initial={{ width: 0 }}
                    animate={{ width: `${Math.min((certCount / 1) * 100, 100)}%` }}
            ...
            <motion.p 
              ...
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              className="text-xs text-white/40 font-sans mt-2"
            >
        ...
        <AnimatePresence>
          {!isLoading && userPlan === 'FREE' && (isProjectFull || isCertFull) && (
            <motion.div 
              ...
              className="bg-rose-950/20 border border-rose-900/30 p-3 rounded-md mt-5 inline-flex items-center gap-2"
            >
        ...
        <AnimatePresence mode="wait">
          {isLoading ? (
            <motion.div 
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="w-full md:w-40 h-12 bg-white/5 border border-white/5 rounded-md animate-pulse shimmer"
            ></motion.div>
          ) : isQuotaFull ? (
            <motion.div
              key="upgrade-btn"
