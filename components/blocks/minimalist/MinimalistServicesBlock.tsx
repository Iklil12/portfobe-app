    ...
    >
      <div className="p-8 @md:p-12 border-b border-gray-200">
        <motion.p variants={cinematicBlurUp} className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2 min-heading">
          <EditableText value={theme?.customTexts?.min_services_label || 'Services'} field="min_services_label" entity="appearance" isEditor={isEditor} as="span" maxLength={20} className="min-heading" />
        </motion.p>
        <motion.h2 variants={cinematicBlurUp} className="text-2xl font-black tracking-tighter uppercase min-heading">
          <EditableText value={theme?.customTexts?.min_services_title || 'What I Do'} field="min_services_title" entity="appearance" isEditor={isEditor} as="span" maxLength={30} className="min-heading" />
        </motion.h2>
      </div>
      ...
      <div className="flex flex-col">
        {services.slice(0, 4).map((service, i) => (
          <motion.div
            ...
            className="p-8 @md:px-12 border-b border-gray-200 last:border-b-0 group cursor-default flex flex-col @md:flex-row @md:items-center @md:justify-between gap-2 @md:gap-8 hover:bg-gray-50 transition-colors duration-300"
          >
            <div className="flex items-center gap-4">
              <span className="text-[10px] font-mono text-gray-300 min-w-[24px]">0{i + 1}</span>
              <h3 className="text-lg @md:text-xl font-black tracking-tighter uppercase min-heading group-hover:translate-x-2 transition-transform duration-300">
                <EditableText value={theme?.customTexts?.[`min_svc_${service.id}_title`] || service.defaultTitle} field={`min_svc_${service.id}_title`} entity="appearance" isEditor={isEditor} as="span" maxLength={40} className="min-heading" />
              </h3>
            </div>
            <p className="text-xs @md:text-sm font-medium text-gray-400 w-full @md:w-1/2 min-body leading-relaxed group-hover:-translate-x-2 transition-transform duration-300">
              <EditableText value={theme?.customTexts?.[`min_svc_${service.id}_desc`] || service.defaultDesc} field={`min_svc_${service.id}_desc`} entity="appearance" isEditor={isEditor} as="span" maxLength={120} className="min-body" />
            </p>
          </motion.div>
        ))}
      </div>
