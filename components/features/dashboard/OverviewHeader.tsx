...
export function OverviewHeader({ subdomain, avatarUrl, isLoading }: OverviewHeaderProps) {
  ...
  const quickActions = [
    ...
    { label: 'Kanvas', href: '/dashboard/appearance', color: 'text-slate-700', bg: 'bg-white border-slate-200 hover:bg-slate-50 hover:border-slate-300' }
  ];
        ...
        <div className="flex gap-3 pt-2 pb-2 px-1 -mt-2 w-full">
          {quickActions.map((action, idx) => (
             <Link 
                key={idx} 
                href={action.href}
                className={`flex-1 md:flex-none flex items-center justify-center px-6 py-2.5 rounded-full border ${action.bg} ${action.color} shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-0.5 font-bold text-sm`}
             >
