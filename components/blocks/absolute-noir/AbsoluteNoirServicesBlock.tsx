...
export const AbsoluteNoirServicesBlock = ({ data, theme, isEditor, isCardPreview }: any) => {
    ...
    // Murni statis, diubah lewat EditableText sesuai standar Pageblock Noir
    const services = [
        ...
        <motion.section initial="hidden" {...{ [animationTrigger]: "visible" }} viewport={{ once: true, amount: 0 }} variants={staggerGrid} className="w-full bg-[#050505] text-white flex flex-col">
            <motion.div variants={wireframeReveal} className="w-full py-4 px-6 wire-border-b flex items-center bg-[#0a0a0a]">
                <span className="font-mono text-sm uppercase tracking-widest">
                    <EditableText value={theme?.customTexts?.noir_services_title || '[ AVAILABLE_PROTOCOLS ]'} field="noir_services_title" entity="appearance" isEditor={isEditor} maxLength={30} as="span" />
                </span>
            </motion.div>
                ...
                {services.map((svc: any, i: number) => {
                    ...
                    return (
                        <motion.div 
                            ...
                            className={`group flex flex-col p-8 @md:p-12 wire-border-b ${!isLastOdd && i % 2 === 0 ? '@lg:wire-border-r' : ''} ${isLastOdd ? '@lg:col-span-2' : ''} hover-invert transition-colors`}
                        >
                            <div className="flex justify-between items-start mb-6">
                                ...
                                <span className="font-mono text-xs uppercase opacity-70 group-hover:opacity-100 flex items-center gap-2">
                                    BASE: <EditableText value={theme?.customTexts?.[`noir_svc_${svc.id}_price`] || svc.defaultPrice} field={`noir_svc_${svc.id}_price`} entity="appearance" isEditor={isEditor} as="span" maxLength={15} />
                                </span>
                            </div>
                            
                            <h3 className="font-sans text-2xl @md:text-4xl font-black uppercase tracking-tight mb-4">
                                <EditableText value={theme?.customTexts?.[`noir_svc_${svc.id}_title`] || svc.defaultTitle} field={`noir_svc_${svc.id}_title`} entity="appearance" isEditor={isEditor} as="span" maxLength={40} />
                            </h3>
                            <div className="font-mono text-sm text-white/60 group-hover:text-black leading-relaxed">
                                <EditableText value={theme?.customTexts?.[`noir_svc_${svc.id}_desc`] || svc.defaultDesc} field={`noir_svc_${svc.id}_desc`} entity="appearance" isEditor={isEditor} as="p" maxLength={150} />
                            </div>
                        </motion.div>
                    );
                })}
