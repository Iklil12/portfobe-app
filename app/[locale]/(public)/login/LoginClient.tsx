...
export default function LoginClient() {
  ...
  const handleResendOtp = async () => {
    ...
    if (result?.error) {
      ...
      setResendCooldown(120);
      setErrorMsg("New verification code sent!");
    }
  };
      ...
      {showForgotModal && (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4">
          ...
          <div className="relative bg-zinc-950 border border-white/10 rounded-none p-8 md:p-10 max-w-md w-full shadow-2xl animate-in zoom-in-95 fade-in duration-200 z-10">
            ...
            <h3 className="text-xl font-display font-bold text-white mb-2 uppercase tracking-wider">{t('forgotPassword')}</h3>
            ...
            <form onSubmit={handleForgotPassword} className="flex flex-col gap-5">
              <div>
                <label className="block text-[9px] font-mono font-bold uppercase tracking-widest text-white/40 mb-2">{t('emailLabel')}</label>
              ...
              >
                {isForgotLoading ? <div className="w-3.5 h-3.5 border-2 border-black/20 border-t-black rounded-full animate-spin"></div> : t('sendResetLink')}
              </button>
            </form>
          </div>
        </div>
        ...
        <div className="w-full max-w-md mx-auto py-12 mt-12 md:mt-0 relative z-10">
          <div className="mb-10">
             <div className="flex justify-between items-center mb-4">
               <div className="inline-flex items-center gap-2 px-3 py-1 border border-[#ff9e00]/20 bg-[#ff9e00]/5 text-[#ff9e00] text-[9px] font-mono uppercase tracking-[0.2em]">
                 Sign In
               </div>
               <LanguageSwitcherPublic />
             </div>
             <h1 className="text-3xl font-display font-bold text-white tracking-tight">{t('title')}</h1>
             <p className="text-white/40 text-xs font-mono mt-2 leading-relaxed">{t('description')}</p>
          </div>
          ...
          {unverifiedEmail ? (
            <form onSubmit={handleVerifyOtp} className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="mb-6">
                <p className="text-white/60 text-xs font-mono leading-relaxed">
                  {t('emailNotVerified')} <br/>
                  <span className="text-[#ff9e00] font-bold">{unverifiedEmail}</span>.
                </p>
              </div>
              <div className="group">
                <label className="block text-[9px] font-mono font-bold uppercase tracking-widest text-white/45 mb-2 group-focus-within:text-[#ff9e00] transition-colors">{t('sixDigitCode')}</label>
                <input 
                  name="otp" 
                  type="text" 
                  maxLength={6}
              ...
              >
                <div className={`flex items-center justify-center gap-2 transition-opacity duration-300 ${isLoading ? 'opacity-0' : 'opacity-100'}`}>
                  {t('verifySignIn')} <ArrowRight className="w-4 h-4" />
                </div>
                {isLoading && (
            ...
            <div className="group">
              <label className="block text-[9px] font-mono font-bold uppercase tracking-widest text-white/45 mb-2 group-focus-within:text-[#ff9e00] transition-colors">{t('emailLabel')}</label>
              <input 
                name="email" 
                type="email" 
            ...
            <div className="group">
              <div className="flex justify-between items-center mb-2">
                <label className="block text-[9px] font-mono font-bold uppercase tracking-widest text-white/45 group-focus-within:text-[#ff9e00] transition-colors">{t('passwordLabel')}</label>
            ...
            >
              <div className={`flex items-center justify-center gap-2 transition-opacity duration-300 ${isLoading ? 'opacity-0' : 'opacity-100'}`}>
                {t('signInButton')} <ArrowRight className="w-4 h-4" />
              </div>
          ...
          <div className="flex items-center my-8">
            ...
            <span className="px-4 text-[9px] font-mono font-bold text-white/20 uppercase tracking-widest">{t('orSignInWith')}</span>
            <div className="flex-grow border-t border-white/5"></div>
          </div>
          ...
          <p className="text-center text-xs text-white/40 mt-10 font-mono">
            {t('noAccount')} <Link href="/register" className="text-white font-bold hover:text-[#ff9e00] transition-colors ml-1 uppercase tracking-wider">{t('registerNow')}</Link>
          </p>
        </div>
      </div>
        ...
        <div className="relative z-10 w-full max-w-md">
          <div className="bg-zinc-950 border border-white/15 rounded-none p-10 shadow-none relative overflow-hidden group hover:border-white/25 hover:shadow-[8px_8px_0px_rgba(255,158,0,0.15)] transition-all duration-500">
            ...
            <div className="inline-flex items-center gap-2 px-3 py-1 border border-white/15 bg-white/5 text-[9px] font-mono font-bold uppercase tracking-[0.2em] text-white/60 mb-8">
              <div className="w-1.5 h-1.5 rounded-none bg-[#ff9e00] animate-pulse"></div> {t('featuredCreator')}
            </div>
            ...
            <div className="flex items-center gap-4">
               ...
               <div>
                  ...
                  <p className="text-[#ff9e00] text-[10px] font-mono font-bold mt-1 uppercase tracking-widest">{t('photographer')}</p>
               </div>
            </div>
          </div>
        </div>
